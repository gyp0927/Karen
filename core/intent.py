...
def _rule_classify(query: str) -> IntentResult | None:
    ...
    if len(q) <= 15 and _match_patterns(q, _GREETING_PATTERNS):
        return IntentResult(
            IntentType.GREETING, confidence=0.95, skip_search=True, skip_memory=True, skip_knowledge=True
        )

    # 2. 告别
    if len(q) <= 15 and _match_patterns(q, _FAREWELL_PATTERNS):
        return IntentResult(
            IntentType.FAREWELL, confidence=0.95, skip_search=True, skip_memory=True, skip_knowledge=True
        )

    # 3. 感谢
    if len(q) <= 15 and _match_patterns(q, _THANKS_PATTERNS):
        return IntentResult(IntentType.THANKS, confidence=0.95, skip_search=True, skip_memory=True, skip_knowledge=True)

    # 4. 数学计算
    if _match_patterns(q, _MATH_PATTERNS):
        return IntentResult(IntentType.MATH, confidence=0.85, skip_search=True, skip_memory=True, skip_knowledge=True)
    ...
    if _match_keywords(q, _TRANSLATION_KEYWORDS):
        return IntentResult(
            IntentType.TRANSLATION, confidence=0.80, skip_search=True, skip_memory=False, skip_knowledge=True
        )

    # 7. 创意写作
    if _match_keywords(q, _CREATIVE_KEYWORDS):
        return IntentResult(
            IntentType.CREATIVE, confidence=0.80, skip_search=True, skip_memory=True, skip_knowledge=True
        )

    # 8. 对比 - 一般知识类比较模型自己就会，不浪费 4-5s 等 web search 超时
    if _match_keywords(q, _COMPARISON_KEYWORDS):
        return IntentResult(
            IntentType.COMPARISON, confidence=0.75, skip_search=True, skip_memory=True, skip_knowledge=True
        )

    # 9. 事实查询（明确需要搜索）
    if _match_keywords(q, _FACTUAL_KEYWORDS):
        return IntentResult(
            IntentType.FACTUAL, confidence=0.75, skip_search=False, skip_memory=False, skip_knowledge=False
        )

    # 10. 闲聊
    if len(q) <= 20 and _match_patterns(q, _CHITCHAT_PATTERNS):
        return IntentResult(
            IntentType.CHITCHAT, confidence=0.80, skip_search=True, skip_memory=True, skip_knowledge=True
        )
...
def _context_classify(query: str, history: list[BaseMessage]) -> IntentResult | None:
    """结合历史消息判断意图。
...
def _infer_parent_intent(history: list[BaseMessage]) -> IntentResult | None:
    """从历史消息推断上一轮的主要意图。"""
    # 找最近的用户消息（排除当前这条）
